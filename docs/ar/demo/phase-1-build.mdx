...
description: >-
  نشر منطقة DNS لـ F5 Distributed Cloud باستخدام Terraform — ملفات الخطة،
  المتغيرات والأسرار المطلوبة، الحالة البعيدة في Azure، وسير عمل GitHub Actions.
sidebar:
  order: 2
...
بحالة بعيدة في Azure Blob Storage. تعرض هذه الصفحة كل ملف في الخطة، وتشرح المتغيرات والأسرار المطلوبة،
وتغطي كلًّا من التشغيل المحلي وخط أنابيب GitHub Actions.

## ما الذي يُنشر

- منطقة `xcsh_dns_zone` للنطاق المفوَّض لديك، في مساحة الاسم `system`، مع مجموعة سجلات A باسم `demo-records`
  (`www` و`app` و`api`).
...
اسم المنطقة نطاقًا يمتلكه حسابك، ويجب أن تكون قيم TTL للسجلات 60 ثانية على الأقل.
</Aside>
...
يُثبّت إصدار Terraform والموفِّر. قيد الموفِّر هو `>= 3.62.0` — الإصدار الذي أصبحت فيه موارد DNS
...
خلفية `azurerm` **جزئية**: لا توجد قيم خاصة بالبيئة مُضمَّنة في الكود. يتم توفير الإحداثيات عند
...
`domain` مطلوب (يُوفَّر في وقت التشغيل، غير مُضمَّن أبدًا)؛ يربط `a_records` كل اسم سجل بعناوين
...
استبدلها بأهداف حقيقية للعرض التوضيحي المباشر.
</Aside>
...
تُنشئ الوحدة المنطقة (يُحذف مساحة الاسم — يُعيِّنها الموفِّر افتراضيًا إلى `system`). يُحوِّل
...
لا يوجد أي قيمة خاصة بالبيئة مُضمَّنة في ملفات `.tf`. يُوفَّر كل شيء في وقت التشغيل — من
**متغيرات** و**أسرار** GitHub Actions في CI، أو من الملفات المحلية ومتغيرات البيئة.
...
<Aside type="caution">
تستخدم مصادقة الخلفية مفتاح وصول حساب التخزين (`ARM_ACCESS_KEY`) بدلًا من OIDC بلا مفتاح / Azure AD.
يتطلب وصول مستوى البيانات بلا مفتاح دور *Storage Blob Data Contributor*، وهو ما لا يستطيع اشتراك
Contributor فقط تعيينه. يوجد مفتاح الوصول فقط في البيئة أو في سر GitHub — ولا يُودَع أبدًا.
</Aside>

## تمهيد خلفية حالة Azure

لا يمكن لتخزين الخلفية تخزين تمهيد نفسه، لذا أنشئه مرة واحدة خارج النطاق باستخدام جلسة Azure CLI
...
[المرحلة 2 — التحقق](/dns/ar/demo/phase-2-validate/) للتأكد من أنها تُحلِّل بشكل صحيح.

## التكامل المستمر

يُشغِّل سير العمل `.github/workflows/terraform.yml` عملية **تخطيط** عند كل طلب سحب وعملية **تطبيق**
...
الإجراءات مثبَّتة على تجزئات الالتزام (مع تعليقات الإصدار)، والأسرار محدودة النطاق لخطوات `init`
و`plan` و`apply` بدلًا من الوظيفة بأكملها.
