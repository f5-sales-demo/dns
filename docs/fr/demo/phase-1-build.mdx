...
La Phase 1 déploie une **zone DNS primaire faisant autorité** sur F5 Distributed Cloud avec Terraform, adossée
à un état distant dans Azure Blob Storage. Cette page présente chaque fichier du plan, explique les variables
et secrets requis, et couvre à la fois une exécution locale et le pipeline GitHub Actions.

## Ce que vous déployez

- Une `xcsh_dns_zone` pour votre domaine délégué, dans le namespace `system`, avec un groupe `demo-records`
  d'enregistrements A (`www`, `app`, `api`).
- L'état Terraform stocké dans un conteneur Azure Blob Storage, afin que le même état soit partagé entre votre
  machine et la CI.

<Aside type="note">
Les objets DNS doivent être créés dans le namespace `system` — l'API F5 XC rejette les autres namespaces. Le
nom de zone doit être un domaine appartenant à votre compte, et les TTL des enregistrements doivent être d'au
moins 60 secondes.
</Aside>
...
Fixe les versions de Terraform et du fournisseur. La contrainte du fournisseur est `>= 3.62.0` — la version
...
Le fournisseur ne prend aucun argument dans le code — il s'authentifie depuis l'environnement, donc aucun secret
n'est validé dans le dépôt.
...
Un backend `azurerm` **partiel** : aucune valeur spécifique à l'environnement n'est codée en dur. Les coordonnées
...
`domain` est requis (fourni à l'exécution, jamais codé en dur) ; `a_records` associe chaque nom d'enregistrement
...
<Aside type="tip">
Les enregistrements A par défaut utilisent des adresses de documentation RFC 5737 (`203.0.113.0/24`), qui ne
sont pas routables. Remplacez-les par de vraies cibles pour une démonstration en conditions réelles.
</Aside>
...
Le module crée la zone (le namespace est omis — le fournisseur le définit par défaut à `system`). Un bloc
...
Aucune valeur spécifique à l'environnement n'est intégrée dans les fichiers `.tf`. Tout est fourni à l'exécution
...
<Aside type="caution">
L'authentification du backend utilise la clé d'accès du compte de stockage (`ARM_ACCESS_KEY`) plutôt qu'une
authentification sans clé OIDC / Azure AD. L'accès au plan de données sans clé nécessite le rôle *Storage Blob
Data Contributor*, qu'un abonnement limité au rôle Contributeur ne peut pas attribuer. La clé d'accès ne vit
que dans l'environnement ou dans un secret GitHub — elle n'est jamais validée dans le dépôt.
</Aside>

## Initialisation du backend d'état Azure

Le stockage du backend ne peut pas stocker son propre bootstrap, il faut donc le créer une seule fois, hors
...
Exportez ensuite la clé pour les exécutions locales, et définissez-la (ainsi que les identifiants du fournisseur)
...
1. **Configurez les entrées.** Copiez les exemples et renseignez vos valeurs :
...
3. **Initialisez** le backend avec la configuration partielle :
...
4. **Vérifiez le formatage et validez :**
...
Une application réussie crée la zone et écrit l'état dans le conteneur Azure. Passez à la
[Phase 2 — Validation](/dns/fr/demo/phase-2-validate/) pour confirmer la résolution.

## Intégration continue

Le workflow `.github/workflows/terraform.yml` exécute un **plan** à chaque pull request et un **apply**
...
Les actions sont épinglées à des SHA de commit (avec des commentaires de version), et les secrets sont limités
